  ...
  namespace json
  {
    ...
    void ParserData::handleComma( JSONMap &parent, JSONFile& stream )
    {
      ...
      switch( next )
      {
        ...
        default : /* Error               */ stream.putback( next ) ; nyx::handleError( nyx::Error::InvalidNMP ) ;
      }
    }
    ...
    void ParserData::handleColon( JSONMap &parent, JSONFile& stream )
    {
      ...
      else
      {
        switch( next )
        {
          ...
          case '}': /* End of an object.   */ 
          case ']': /* End of an array.    */ 
          default : /* Could be # or bool  */ stream.putback( next ) ; nyx::handleError( nyx::Error::InvalidNMP ) ;
        }
      }
    }
    ...
    void ParserData::handleArray( JSONMap &parent, JSONFile& stream )
    {
      ...
      while( ( next = getNextValidCharacter( stream ) ) != ']' && next != stream.eof() )
      {
        ...
        switch( next )
        {
          ...
          case ' ': /* White space. Skip.  */
          case '}': /* End of an object.   */
          case ']': /* End of an array.    */ stream.putback( next ) ; nyx::handleError( nyx::Error::InvalidNMP ) ;
        }
      }
    }
    ...
    void ParserData::handleObject( JSONMap &parent,  JSONFile& stream )
    {
      ...
      while( ( next = getNextValidCharacter( stream ) ) != '}' && next != stream.eof() )
      {
        switch( next )
        {
          ...
          case '}': /* End of an object.   */ 
          case ']': /* End of an array.    */ 
          default : /* Error               */ nyx::handleError( nyx::Error::InvalidNMP ) ;
        }
      }
    }
    ...
    void ParserData::handleKey( JSONMap &parent, JSONFile& stream )
    {
      ...
      switch( next )
      {
        ...
        case '}': /* End of an object.   */ 
        case ']': /* End of an array.    */ 
        default : /* Could be # or bool  */ stream.putback( next ) ; stream.putback( next ) ; nyx::handleError( nyx::Error::InvalidNMP ) ;
      }
    }

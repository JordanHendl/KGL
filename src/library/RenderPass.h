...
namespace nyx
{
  ...
  class RenderPass
  {
    public:
       ...
       * @param context The Framework context for this Pipeline to draw to.
       */
       ...
       * @param render_chain The chain to submit as a part of this presentation. The implementation will queue the GPU
       *  to wait on the operation before presenting to the screen.
       * @return Whether or not this render pass had to reallocate data based on window events.
       */
      bool present( nyx::Chain<Framework>& render_chain ) ;
  ...
  template<typename Framework>
  bool RenderPass<Framework>::present( nyx::Chain<Framework>& render_chain )
  {
    return this->impl.present( render_chain ) ;
  }
